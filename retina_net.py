...
class ResNet(nn.Layer):
    ...
    def forward(self, inputs):
        ...
        if self.training:
            return self.focalLoss(classification, regression, anchors, annotations)#此处采用focalloss
        else:
            ...
            for i in range(classification.shape[2]):
                scores = paddle.squeeze(classification[:, :, i])
                ...
                anchorBoxes = paddle.squeeze(transformed_anchors)
                anchorBoxes = anchorBoxes[scores_over_thresh]
                anchors_nms_idx = matrix_nms(anchorBoxes, scores, 0.5)
...
def resnet50(num_classes, pretrained=False, **kwargs):
    ...
    if pretrained:
        model.get_weights_path_from_url('https://paddle-hapi.bj.bcebos.com/models/resnet50.pdparams')
    return model
